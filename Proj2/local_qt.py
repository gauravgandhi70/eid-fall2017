...
class Ui_Dialog(QtWidgets.QWidget): 
	...
	def setupUi(self, Dialog):
		...
		self.radioButton_2.setGeometry(QtCore.QRect(600, 50, 119, 23))
	...
	def timeout_isr(self):
		...
		if(temp != None):
			...
			with open('Weather_Data.csv', 'a') as csvfile:
				...
				filewriter.writerow(['{0:.2f}'.format(humidity),'{0:.2f}'.format(temp),'{0:.2f}'.format(self.avg_h),'{0:.2f}'.format(self.avg_t)])
		else:
	...
	def query_temp(self):
		...
		if(temp == None):
			...
			with open('Weather_Data.csv', 'a') as csvfile:
				filewriter = csv.writer(csvfile, delimiter=',',quotechar='|', quoting=csv.QUOTE_MINIMAL)
				filewriter.writerow([humidity_string,temp_string,'{0:.2f}'.format(self.avg_h),'{0:.2f}'.format(self.avg_t)])
	...
	def plot_graph(self):
		humidity,temp,avgt,avgh = np.loadtxt('Weather_Data.csv', delimiter=',', unpack=True)
